...
const roleButtonClass = "cursor-pointer rounded-full border-0 px-[17px] py-[11px] text-xs font-bold";
    ...
    <section className="relative flex min-h-[850px] items-center overflow-hidden bg-[radial-gradient(circle_at_77%_20%,#0a55c7_0,transparent_30%),linear-gradient(130deg,#06183f_0%,#052766_50%,#063582_100%)] pb-[90px] pt-[145px] text-white max-[680px]:min-h-0 max-[680px]:pb-[70px] max-[680px]:pt-[120px]">
      ...
      <div className="relative z-[2] mx-auto grid w-[min(1180px,calc(100%-40px))] grid-cols-[1fr_500px] items-center gap-[90px] max-[980px]:grid-cols-1 max-[980px]:gap-[45px] max-[680px]:w-[min(1180px,calc(100%-28px))]">
        <div className="max-[980px]:text-center">
          <span className="inline-block text-xs font-black uppercase tracking-[.16em] text-[#8fc1ff]">Code World icması</span>
        ...
        <form className="rounded-[25px] bg-white p-[38px] text-[#0d1119] shadow-[0_35px_85px_rgba(0,0,0,.3)] max-[980px]:mx-auto max-[980px]:w-[min(100%,520px)] max-[680px]:px-5 max-[680px]:py-7" onSubmit={handleSubmit}>
          ...
          <div className="mb-[22px] grid grid-cols-2 gap-[7px] rounded-full bg-[#f0f2f5] p-[5px]" aria-label="Qeydiyyat növü">
            <button type="button" className={`${roleButtonClass} ${role === "student" ? "bg-[#0869f7] text-white" : "bg-transparent text-[#616875]"}`} onClick={() => setRole("student")}>Tələbə</button>
            <button type="button" className={`${roleButtonClass} ${role === "intern" ? "bg-[#0869f7] text-white" : "bg-transparent text-[#616875]"}`} onClick={() => setRole("intern")}>Təcrübəçi</button>
          </div>
          <label className="mb-[17px] grid gap-2 text-xs font-extrabold text-[#424956]">Ad və soyad<input className={inputClass} name="name" type="text" placeholder="Ad Soyad" minLength={2} required /></label>
